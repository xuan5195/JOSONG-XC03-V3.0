...
*	修改记录 :
*		版本号  日期        作者     说明
*		V1.0    20130201 armfly  正式发布
*		V1.1    20130629 armfly  增加1个读指针，用于bsp_Idle() 函数读取系统控制组合键（截屏）
*								   增加 K1 K2 组合键 和 K2 K3 组合键，用于系统控制
*		V1.2    20150808 armfly  K1，K2，K3独立按键进行排他判断，修改  IsKeyDown1()等函数
*		V1.3    20180628 北宸    统一整理，归九松电子使用
*
...
/*
	使用说明：
		...
		2.读按键值：
			uint8_t ucKeyCode;
			ucKeyCode = bsp_GetKey();	//读取按键值，注意支持连发时，要及时读取数值，否则先按下的值会丢；
		3.定时扫描按键
