...
*	修改补充 : 
*		版本号   日期       作者          说明
*		V1.0    2015-08-03 Eric2013   正式发布, 对于中断开关和中断服务程序加入uCOS-II处理
*
...
uint8_t RS485_Count;
uint8_t RS485Dat[10];
uint8_t g_SerialDat[32];
uint8_t g_Serial_Count;
...
static uint8_t RS485ADD1(void) {if ((GPIOD->IDR & GPIO_Pin_11) == 0) return 1;else return 0;}
static uint8_t RS485ADD2(void) {if ((GPIOD->IDR & GPIO_Pin_10) == 0) return 1;else return 0;}
static uint8_t RS485ADD3(void) {if ((GPIOD->IDR & GPIO_Pin_9 ) == 0) return 1;else return 0;}
static uint8_t RS485ADD4(void) {if ((GPIOD->IDR & GPIO_Pin_8 ) == 0) return 1;else return 0;}
static uint8_t RS485ADD5(void) {if ((GPIOE->IDR & GPIO_Pin_11) == 0) return 1;else return 0;}
static uint8_t RS485ADD6(void) {if ((GPIOE->IDR & GPIO_Pin_10) == 0) return 1;else return 0;}
static uint8_t RS485ADD7(void) {if ((GPIOE->IDR & GPIO_Pin_9 ) == 0) return 1;else return 0;}
static uint8_t RS485ADD8(void) {if ((GPIOE->IDR & GPIO_Pin_8 ) == 0) return 1;else return 0;}

void RS485_InitTXE(void);

void bsp_InitRS485ADD(void)
{
...
{
	uint8_t Add=0;
	if( RS485ADD1() == 1 )	Add |= (1<<0);	else Add &= ~(1<<0);
	if( RS485ADD2() == 1 )	Add |= (1<<1);	else Add &= ~(1<<1);
	if( RS485ADD3() == 1 )	Add |= (1<<2);	else Add &= ~(1<<2);
	if( RS485ADD4() == 1 )	Add |= (1<<3);	else Add &= ~(1<<3);
	if( RS485ADD5() == 1 )	Add |= (1<<4);	else Add &= ~(1<<4);
	if( RS485ADD6() == 1 )	Add |= (1<<5);	else Add &= ~(1<<5);
	if( RS485ADD7() == 1 )	Add |= (1<<6);	else Add &= ~(1<<6);
	if( RS485ADD8() == 1 )	Add |= (1<<7);	else Add &= ~(1<<7);
	return Add;
}
...
void ReceivePowerBUSDat(uint8_t _mode)
{
    ...
    for(i=0;i<16;i++)
    {
        if (comGetChar(COM2, &read))    //接收到串口数据
        {
            if((PBus_Count&0x80)==0)//接收未完成；
            {
                PBusDat[PBus_Count] = read;
                PBus_Count++;
                if(PBus_Count>16)   PBus_Count = 0;
                if(PBusDat[PBus_Count-1] == 0xF3)   //判断起始位0xF3;
                {
                    PBusDat[PBus_Count-1] = 0xF3;
                    PBus_Count = 1;
                }
                else
                {
                    if(_mode==0x00) //不带校验模式；
                    {
                        PBus_Count|=0x80;   //接收完成
                        break;
                    }
                    else    //带核验模式
                    {
                        if(PBus_Count>=PBusDat[1])
                        {
    ...
    uint8_t read,CRC_Dat;
		if (comGetChar(COM3, &read))    //接收到串口数据
		{
			if((g_Serial_Count&0x80)==0)//接收未完成；
			{
				...
				if(g_SerialDat[g_Serial_Count-1] == 0xF3)   //判断起始位0xF3;
				{
...
void RS485_ReceiveDat(void)
{
    uint8_t read,CRC_Dat;
    uint16_t i;
    for(i=0;i<UART3_TX_BUF_SIZE;i++)
    {
         if (comGetChar(COM3, &read))
        {
           if((RS485_Count&0x80)==0)//接收未完成；
            {
                RS485Dat[RS485_Count] = read;
                RS485_Count++;
                if(RS485_Count>10)   RS485_Count = 0;
                if((RS485Dat[RS485_Count-1] == 0xF2)&&(RS485_Count<3))   //判断起始位0xF1;
                {
                    RS485Dat[RS485_Count-1] = 0xF2;
                    RS485_Count = 1;
                }
				else
				{
					if(RS485_Count>=RS485Dat[1])
					{
						CRC_Dat = CRC8_Table(RS485Dat,6);
						if(CRC_Dat == RS485Dat[6])	RS485_Count|=0x80;	/*接收完成了 */
						else	RS485_Count = 0;
					}
//					if((RS485Dat[RS485_Count-2] == 0x03)&&(RS485Dat[RS485_Count-1] == 0x02))
//					{
//						RS485_Count|=0x80;   //接收完成
//						break;
//					}
				}
            }            
...
void RS485_SendDat(uint8_t *_uBuff)
{
	uint8_t Runningbuf[9]={0xF1,0x09,0x01,0x00,0x00,0x00,0x00,0x03,0x02};
	Runningbuf[2] = _uBuff[0];	
	Runningbuf[3] = _uBuff[1];	
	Runningbuf[4] = _uBuff[2];	
	Runningbuf[5] = _uBuff[3];	
	Runningbuf[6] = CRC8_Table(Runningbuf,6);	//CRC校验位	
	RS485_SendBuf((uint8_t *)Runningbuf,Runningbuf[1]);
}
...
static void InitHardUart(void)
{
	...
	*/
    ...
    //GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF_USART4);
